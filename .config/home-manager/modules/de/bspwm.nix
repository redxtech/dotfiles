...
let
  ...
  runOnceDesktop =  cmd: "! pgrep -f ${cmd} && dex ${config.xdg.dataHome}/applications/${cmd}.desktop";
  runWithRule = {cmd, window, rule}: "${pkgs.bspwm}/bin/bspc rule -a ${window} -o ${rule} && ${cmd}";
in {
  xsession.windowManager.bspwm = with builtins; {
    ...
    rules = {
      ...
      discord = { desktop = "chat"; follow = false; };
    ...
    ++
    map runWithRule [
      { cmd = "${kitty}/bin/kitty ${btop}/bin/btop"; window = "kitty"; rule = "state=floating desktop='^7'"; }
    ];
  };

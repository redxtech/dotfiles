...
''
  ...
  class PlayerManager:
      ...
      def onChangedProperties(
          ...
          if sender in self.players:
              ...
              # If we know this player,
              # but haven't been able to set up a signal handler
      ...
      def getBusNameFromOwner(self, owner):
          ...
          for player_bus_n in player_bus_names:
              player_bus_owner = self._session_bus.get_name_owner(player_bus_n)
  ...
  class Player:
      ...
      def refreshStatus(self):
          # Some clients (VLC) will momentarily create a
          # new player before removing it again
      ...
      def refreshMetadata(self):
          # Some clients (VLC) will momentarily create
          # a new player before removing it again
          # so we can't be sure the interface still exists
          try:
      ...
      def _parseMetadata(self):
          if self._metadata is not None:
              ...
              # Formats using h:mm:ss if length > 1 hour, else m:ss
              _fmt_length = (
                  f"{_length_int/1e6//60:.0f}:{_length_int/1e6%60:02.0f}"
                  if _length_int < 3600 * 1e6
                  else f"{_length_int/1e6//3600:.0f}:"
                  + f"{_length_int/1e6%3600//60:02.0f}:"
                  + f"{_length_int/1e6%60:02.0f}"
              )
      ...
      def printStatus(self):
          if self.status in ["playing", "paused"]:
              ...
              # restore polybar tag formatting and replace
              # any remaining metadata tags after that
              try:
                  text = re.sub(
                      r"􏿿p􏿿(.*?)􏿿p􏿿(.*?)􏿿p􏿿(.*?)􏿿p􏿿",
                      r"%{\1}\2%{\3}",
                  ...
                  print("Invalid format string:", ex)
              self._print(text)
          else:
  ...
  def _metadataGetFirstItem(_value):
      if type(_value) is list:
          ...
          # item on _value if it has at least one item.
          # Returns an empty string if _value is empty.
          return str(_value[0]) if len(_value) else ""
      else:
          # If _value isn't a list just return the
  ...
  parser.add_argument(
      ...
      help=" ".join(
          [
              "ignore a player by it's bus name.",
              "Can be given multiple times (e.g. -b vlc -b audacious)",
          ]
      ),
      action="append",
      metavar="BUS_NAME",
  ...
  parser.add_argument(
      ...
      help=" ".join(
          [
              "permit a player by it's bus name like --blacklist.",
              "will block --blacklist if given",
          ]
      ),
      action="append",
      metavar="BUS_NAME",
